...
const NewCourseData = () => {
  ...
  return (
    <section className="grow">
      ...
      <p className="xl:w-[40%] mx-auto font-medium text-xl mb-6">Course Name: {newCourse.course.courseName}</p>
      <p className="xl:w-[40%] mx-auto font-medium text-xl mb-6">Course Code: {newCourse.course.courseCode}</p>
      <p className="xl:w-[40%] mx-auto font-medium text-xl mb-6">Doctor: {newCourse.doctorName}</p>
      ...
      <div className="flex flex-wrap gap-4 mb-[180px] xl:w-[40%] mx-auto">
        {newCourse.course.prerequisites.length !== 0 &&
          newCourse.course.prerequisites.map((course, index) => (
            <div className="text-white bg-[#575AA2] py-2 px-4 rounded-lg capitalize" key={index}>
              {course}
            </div>
          ))}
        {newCourse.course.prerequisites.length === 0 && <p className="text-red-500 text-lg">No Prerequisites Found</p>}
      </div>
